...
LOGS="$(dirname $0)/logs"
$(dirname $0)/../common/mk_folder.sh "$LOGS"
...
prepare(){
	[[ -f "$(dirname $0)/_prepare.sh" ]] || return 0
	$(dirname $0)/../common/exe_on_board.sh "_prepare.sh"
}
...
rmOldLog(){
	if [[ -f "$log_file" ]]; then
		if [[ "$ENV" == "board" ]]; then
			...
			if [[ -n "$SUDO_PW" ]]; then
				echo "$SUDO_PW" | sudo -S sh -c "chmod 666 '$log_file'"
			else
				sudo sh -c "chmod 666 '$log_file'"
			fi
			rm "$log_file"
		fi
	fi
