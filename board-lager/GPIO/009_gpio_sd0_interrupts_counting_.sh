...
echo "checking gpio sd interrupt counting when inserting sd card 0"

# constant
CHECK_INTERRUPT="$(dirname $0)/../common/check_interrupt_count.sh"
cache="gpio_sd0_interrupts_counting.tmp"

# todo
$(dirname $0)/../common/wait_user_action.sh "Ensure that you REMOVE ALL the sd card then input \"next\" to continue..." "next"
...
#wait user action
$(dirname $0)/../common/wait_user_action.sh "\nPlease INSERT the sd card 0 then input \"next\" to continue..." "next"
...
#wait user action
$(dirname $0)/../common/wait_user_action.sh "\n\nPlease REMOVE the sd card 0 then input \"next\" to continue..." "next"
