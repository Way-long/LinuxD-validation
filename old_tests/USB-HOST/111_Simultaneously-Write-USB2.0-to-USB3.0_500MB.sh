...
if ls /dev | grep ${DEVICE_USB2C1} > /dev/null 2>&1; then
	$(dirname $0)/usb_simultaneously_copy_inside_two_usb.sh $USB2_DIR $USB3_DIR 500
else
	if ls /dev | grep ${DEVICE_USB2C2} > /dev/null 2>&1; then
		$(dirname $0)/usb_simultaneously_copy_inside_two_usb.sh $USB2_CH2_DIR $USB3_DIR 500
	else
